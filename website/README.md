...
Static deployment check file:

- `/version.json`

`version.json` is generated by `npm run version:generate`, which also runs automatically before `npm run build`. It includes the package version and current git commit so each deployed commit can be identified.

The GitHub Actions workflow for Azure Static Web Apps is stored at `.github/workflows/azure-static-web-apps-calm-dune-0d3fde11e.yml`.
